...
if (doPlotSensorNetwork)
    ...
    r_m = round(num_of_sensor(2)/2);
    c_m = round(num_of_sensor(2)/2);
    current_pos = [r_m; c_m];
    window_size = [3 3];
    ...
    for r = 1 : size(sensor_neighbourhood, 1)
        for c = 1 : size(sensor_neighbourhood, 2)
            neighbour_plot = plot(sensor_neighbourhood(r,c).x,...
                          sensor_neighbourhood(r,c).y,...
                           'MarkerSize', 10, ...
                           'MarkerFaceColor', [0.5176 1.0000 0], ...
                           'MarkerEdgeColor', 'red', ...
                           'Marker', 'diamond');
        end
    end
    active_sensor_plot = plot(sensor_network(r_m,c_m).x,...
                              sensor_network(r_m,c_m).y,...
                               'MarkerSize', 10, ...
                               'MarkerFaceColor', [1 0.4784 0.4784], ...
                               'MarkerEdgeColor', 'red', ...
                               'Marker', 'diamond');
    ...
    h = legend([sensor_plot, active_sensor_plot, neighbour_plot], ...
    'Sensor', 'Active Sensor', 'Neighbour Sensor', ...
    'Location', 'northeast');
    set(h,'FontSize',15);
end
